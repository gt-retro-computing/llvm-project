...
static int64_t getSubRegIndex(const TargetRegisterClass *RC, unsigned Off = 0) {
  switch (Off) {
  case 0:
    if (RC == &Z80::R16RegClass)
      return Z80::sub_short;
    if (RC == &Z80::R8RegClass)
      return Z80::sub_low;
    break;
  case 8:
    if (RC == &Z80::R8RegClass)
    ...
    if (DstSize > SrcSize && SrcRegBank.getID() == Z80::GPRRegBankID &&
      ...
      if (SrcRC != DstRC)
        ...
        I.getOperand(1).setReg(
            MachineIRBuilder(I)
                .buildInstr(TargetOpcode::SUBREG_TO_REG, {DstRC},
                            {int64_t(0), SrcReg, getSubRegIndex(SrcRC)})
                .getReg(0));
    }
...
bool Z80InstructionSelector::selectAnyExt(MachineInstr &I,
  ...
  assert(RBI.getSizeInBits(DstReg, MRI, TRI) >
  ...
  MachineIRBuilder(I).buildInstr(TargetOpcode::SUBREG_TO_REG, {DstReg},
                                 {int64_t(0), SrcReg, getSubRegIndex(SrcRC)});
  I.eraseFromParent();
  return true;
}
...
static bool canFoldLoad(MachineInstr &LoadMI, MachineInstr &TargetMI) {
  ...
  if (!MBB || TargetMI.getParent() != MBB)
    return false;
  for (MachineBasicBlock::iterator I(LoadMI), E(MBB->end()); I != E; ++I) {
...
bool Z80InstructionSelector::selectExtract(MachineInstr &I,
  ...
  unsigned SubIdx = getSubRegIndex(DstRC, I.getOperand(2).getImm());
