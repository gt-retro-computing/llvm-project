...
bool Z80RegisterInfo::isFrameOffsetLegal(const MachineInstr *MI,
                                         ...
                                         int64_t Offset) const {
  unsigned Idx;
  for (Idx = 0; !MI->getOperand(Idx).isFI(); ++Idx)
    assert(Idx + 2 < MI->getNumOperands() &&
           "Instr doesn't have FrameIndex operand!");
  Offset += getFrameIndexInstrOffset(MI, Idx);
  unsigned Opc = MI->getOpcode();
  return isInt<8>(Offset) &&
         ((Opc != Z80::LD88ro && Opc != Z80::LD88or) || isInt<8>(Offset + 1));
}
